...
// --- Calculate Initial Price (Default: small, 56 pcs) ---
$priceSmall56 = (int)($settings['priceSmall56'] ?? 300);
$deliveryFee = (int)($settings['deliveryCharge'] ?? 70);
$totalPrice = $priceSmall56 + $deliveryFee;
...
<head>
    ...
    <meta name="keywords" content="ইসলামিক দোয়া স্টিকার, দোয়া স্টিকার কার্ড, ইসলামিক স্টিকার, Nawarika, Buy dua sticker card, Islamic sticker Bangladesh, দোয়া কার্ড">
...
<div class="container mx-auto p-4 md:p-8 max-w-4xl">
        ...
        <main class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
                ...
                <div id="spin-container" <?php if (($settings['isSpinningWheelEnabled'] ?? '1') === '0') echo 'style="display:none;"'; ?>>
                ...
                <div id="customer-feedback-section" class="mt-8 space-y-6 hidden">
                 ...
                 <section id="recent-orders-section" class="mt-12 p-6 bg-white rounded-lg shadow-lg max-w-4xl mx-auto hidden">
                    <h2 class="text-3xl font-bold text-center text-green-800 mb-2">🌸 আমাদের সম্মানিত দোয়া স্টিকার ক্রেতাদের তালিকা</h2>
                    <p class="text-center text-gray-600 mb-6">এরকম হাজারো পরিবার দেশের প্রতিটি বিভাগ ও জেলার মানুষ আমাদের পণ্যের উপর আস্থা রাখছেন...</p>
            ...
            <div id="order-section" class="bg-white p-6 rounded-lg shadow-lg">
                ...
                <div id="productOptions" class="my-6 space-y-4">
                    <div>
                        ...
                        <div class="flex gap-4">
                            ...
                            <input type="radio" name="size" id="size_small" value="small" class="hidden" <?php if (($settings['isSmallSizeEnabled'] ?? '1') === '0') echo 'disabled'; else echo 'checked'; ?>>
                            <label for="size_small" class="radio-label text-center w-full" <?php if (($settings['isSmallSizeEnabled'] ?? '1') === '0') echo 'style="display:none;"'; ?>>
                                <?php echo htmlspecialchars($settings['smallSizeLabel'] ?? 'ছোট'); ?>
                            </label>
                            <input type="radio" name="size" id="size_big" value="big" class="hidden" <?php if (($settings['isBigSizeEnabled'] ?? '1') === '0') echo 'disabled'; if (($settings['isSmallSizeEnabled'] ?? '1') === '0') echo 'checked'; ?>>
                            <label for="size_big" class="radio-label text-center w-full" <?php if (($settings['isBigSizeEnabled'] ?? '1') === '0') echo 'style="display:none;"'; ?>>
                                <?php echo htmlspecialchars($settings['bigSizeLabel'] ?? 'বড়'); ?>
                            </label>
                        </div>
                        <div class="mt-4">
                           <img id="size-image-preview" src="" alt="Selected size preview" class="w-full rounded-lg shadow-md hidden">
                        </div>
                    </div>
                     <div>
                        <h3 class="font-semibold mb-2">পরিমাণ:</h3>
                        <div class="flex gap-4">
                            <input type="radio" name="quantity" id="quantity_32" value="32" class="hidden">
                            <label for="quantity_32" class="radio-label text-center w-full">৩২ পিস</label>
                            <input type="radio" name="quantity" id="quantity_56" value="56" class="hidden" checked>
                ...
                <div class="my-6 text-center bg-gray-100 p-4 rounded-lg space-y-2">
                    <p class="text-lg text-gray-800">দাম: <span id="dynamicPrice" class="font-semibold"><?php echo $priceSmall56; ?></span> টাকা</p>
                    <p class="text-lg text-gray-800">ডেলিভারি চার্জ: <span id="deliveryFee" class="font-semibold"><?php echo $deliveryFee; ?></span> টাকা</p>
                    <hr class="border-gray-300">
                    <p class="text-2xl font-bold text-red-600">সর্বমোট: <span id="totalPrice"><?php echo $totalPrice; ?></span> টাকা</p>
                </div>

                <!-- Order Form - Populated with default data by PHP -->
                <form id="orderForm" class="space-y-4">
                    <input type="hidden" id="selectedProduct" name="selectedProduct" value="<?php echo htmlspecialchars($settings['smallSizeLabel'] ?? 'ছোট'); ?> - ৫৬ পিস">
                    <input type="hidden" id="productPriceInput" name="productPriceInput" value="<?php echo $priceSmall56; ?>">
                   ...
                   <div class="relative">
                        ...
                        <div id="addressSuggestions" class="hidden absolute left-0 right-0 z-10 bg-white border border-gray-300 rounded-b-md max-h-60 overflow-y-auto shadow-lg">
                        </div>
                    </div>
                    <div>
                ...
                <!-- Alert Box -->
                <div role="alert" aria-live="polite" class="max-w-xl mx-auto p-4 rounded-lg bg-gradient-to-r from-yellow-50 via-yellow-100 to-white border-l-4 border-yellow-400 shadow-sm">
                    <div class="items-start gap-4">
                        <div>
                        <p class="text-red-700 font-semibold text-lg">⚠️ সতর্কবার্তা:</p>
                        <p class="mt-1 text-gray-800 leading-relaxed">দয়া করে আল্লাহর দোয়া ও কলাম সংবলিত পণ্য নিয়ে কেউ প্রতারণা বা অর্ডার দিয়ে ডেলিভারি ম্যানের ফোন রিসিভ না করে হয়রানি করবেন না।</p>
                        <p class="text-red-700 font-semibold text-md">এরকম ফ্রড বা হয়রানি করলে আপনার নাম ঠিকানা, ফোন নাম্বার উল্লেখ করে থানায় জিডি করা হবে</p>
                        <p class="mt-2 text-md text-green-600">🙏 যারা সত্যিই নিতে আগ্রহী, শুধুমাত্র তারাই অর্ডার করুন। সম্পূর্ণ ক্যাশ অন ডেলিভারী, পার্সেল খুলে চেক করে নিবেন, আমরা যেরকম বলেছি সেরকম না হলে রিটার্ন করে দিবেন</p>
                        </div>
                    </div>
                </div>
        ...
        <footer class="text-center mt-10 pt-6 border-t">
            ...
            <p class="text-gray-600">সারা দেশে ফ্ল্যাট <strong><?php echo $deliveryFee; ?> টাকা</strong> হোম ডেলিভারি।</p>
            <p class="text-gray-600 mt-2">যেকোনো প্রয়োজনে কল করুন: <strong class="text-green-700"><?php echo htmlspecialchars($settings['supportPhone'] ?? '01884031111'); ?></strong></p>
        </footer>
    </div>
   ...
   <div class="fab-container">
    <div class="fab-item">
        ...
        <a id="messenger-chat-link" href="https://m.me/<?php echo htmlspecialchars($settings['messengerPageId'] ?? ''); ?>" target="_blank" class="chat-fab messenger-fab" 
           style="<?php echo empty($settings['messengerPageId']) ? 'display: none;' : 'display: flex;'; ?>" title="Chat with us on Messenger">
    ...
    <div class="fab-item">
        ...
        <a id="whatsapp-chat-link" href="https://wa.me/<?php echo htmlspecialchars($settings['whatsappNumber'] ?? ''); ?>" target="_blank" class="chat-fab whatsapp-fab" 
           style="<?php echo empty($settings['whatsappNumber']) ? 'display: none;' : 'display: flex;'; ?>" title="Chat with us on WhatsApp">
